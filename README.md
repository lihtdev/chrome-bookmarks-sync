...
> 适用于升级到本版本前已经产生重复的情况。本版本已通过同步并发锁防止**新增**重复，但无法自动清除**已存在**的历史重复（自动去重需判断保留哪一个子树，可能误删用户有意创建的同名文件夹），请按下面任一方式手动清理一次。

**方式一：从本地清理（推荐）**

1. 打开 Chrome 书签管理器，手动删除重复的同名文件夹，每个同名只保留一个。
2. 点击扩展弹窗的「立即同步」。
3. 本地修改会触发「本地比云端新」→ 推送本地到云端，云端 `bookmarks.json` 被覆盖为干净版本。
4. 之后本地与云端均无重复，并发锁会防止再次产生。

**方式二：从云端清理**

1. 在 Gitee 仓库网页直接编辑 `bookmarks.json`，删除重复的同名节点并提交。
2. 触发一次同步（等待自动同步或点击「立即同步」）。
3. 云端哈希变化触发「云端比本地新」→ 拉取云端到本地，本地多余的同名节点会被增量合并的删除逻辑清除。
4. 再次同步确认无新增重复。
